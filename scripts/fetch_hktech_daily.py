...
RAW_DIR = "data_raw/HKTECH"
CLEAN_DIR = "data_clean/HKTECH"

RAW_PATH = f"{RAW_DIR}/hktech_raw.csv"
CLEAN_PATH = f"{CLEAN_DIR}/hktech_clean.csv"
...
def prepare_raw_table(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"hktech raw missing required columns: {missing}")
...
def validate_clean_table(clean: pd.DataFrame) -> None:
    if clean is None or clean.empty:
        raise ValueError("hktech_clean is empty")
    ...
    if missing_cols:
        raise ValueError(f"hktech_clean missing required columns: {missing_cols}")

    null_counts = clean[required_not_null].isna().sum()
    bad_cols = null_counts[null_counts > 0]
    if len(bad_cols) > 0:
        raise ValueError(f"hktech_clean has nulls in required columns: {bad_cols.to_dict()}")

    fail_rows = int((clean["quality_flag"] == "FAIL").sum())
    if fail_rows > 0:
        raise ValueError(f"hktech_clean contains FAIL rows: {fail_rows}")

    if clean["trade_date"].duplicated().any():
        dup_count = int(clean["trade_date"].duplicated().sum())
        raise ValueError(f"hktech_clean has duplicated trade_date rows: {dup_count}")

    if not clean["trade_date"].is_monotonic_increasing:
        raise ValueError("hktech_clean trade_date is not sorted ascending")

    if (clean["symbol"] != SYMBOL).any():
        raise ValueError("hktech_clean symbol contains invalid values")

    if (clean["market"] != MARKET).any():
        raise ValueError("hktech_clean market contains invalid values")

    if (clean["asset_type"] != ASSET_TYPE).any():
        raise ValueError("hktech_clean asset_type contains invalid values")
    ...
    try:
        ...
        if df_raw.empty:
            append_refresh_log(
                ...
                rows_clean=0,
                fail_rows=0,
                message="index_global returned empty dataframe for HKTECH; check tushare permission or ts_code",
            )
            raise ValueError("index_global returned empty dataframe for HKTECH")
        ...
        if rows_raw == 0:
            append_refresh_log(
                ...
                rows_clean=0,
                fail_rows=0,
                message="prepared raw table is empty for HKTECH",
            )
            raise ValueError("prepared raw table is empty for HKTECH")
        ...
        append_refresh_log(
            ...
            rows_clean=rows_clean,
            fail_rows=fail_rows,
            message="hktech full refresh completed via index_global",
        )
