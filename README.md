...
* 提供同业物流代理层
* 为后续京东物流与同业物流环境、快递履约行业景气、相对强弱比较提供正式底层表
* 当前已完成单线 raw / clean 接入，并已纳入仓库级正式成员体系
...
* 提供同业物流代理层补充
* 为后续京东物流与快递网络扩张、价格竞争、行业变化环境识别提供正式底层表
* 当前已完成单线 raw / clean 接入
...
**“京东物流 + 平台生态层 + 同业物流层 + 港股市场基准”的正式底层联动底座阶段。**
...
下一阶段不回头修旧问题，而是先做一次系统复盘，再决定是否继续补新对象。
...
* 现有环境层是否已经足够支撑京东物流走势预测
* 还缺的是对象，还是缺特征设计
* 是否该从底座扩张转向特征层准备

### 第二优先级：继续强化 schema / contract 文档
...
**把京东物流、平台生态层、同业物流层及相关市场基准数据，建设成一套可训练、可回放、可审计、字段完整、结构稳定、可整仓刷新、可整仓验收的正式源数据仓底座。**

2026-03-18 更新：极兔速递-W 代理层 `1519.HK` 已纳入总闸门；refresh_all_and_validate 默认 include_1519_in_gate=1、run_1519_fetch=1。
